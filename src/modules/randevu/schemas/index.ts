...
export const createStaffSchema = z.object({
  ...
  email: z.preprocess(
    ...
    z.string().email("Geçerli e-posta giriniz").optional(),
  ),
  password: z.preprocess(
    (v) => (v === "" || v == null ? undefined : v),
    z.string().min(6, "Şifre en az 6 karakter olmalıdır").optional(),
  ),
  phone: z.preprocess(
    (v) => (v === "" || v == null ? undefined : v),
    z.string().optional(),
...
export const createAppointmentSchema = z
  .object({
    ...
    guestName: z.preprocess(
      (v) => (v === "" || v == null ? undefined : v),
      z.string().min(2, "Ad soyad en az 2 karakter").optional(),
    ),
  ...
  .refine((d) => d.customerId || d.guestName, {
    message: "Kayıtlı müşteri veya misafir adı gerekilidir",
    path: ["guestName"],
  });
...
// ─── StaffBreak ────────────────────────────────────────────────────────────────
export const createStaffBreakSchema = z
  .object({
    staffId: z.string().uuid(),
    dayOfWeek: z.number().min(0).max(6),
    startTime: z.string().regex(timeRegex, "Geçerli saat formatı: HH:mm"),
...
export type CreateStaffBreakInput = z.infer<typeof createStaffBreakSchema>;
