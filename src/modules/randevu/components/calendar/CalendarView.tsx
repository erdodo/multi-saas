...
export default function CalendarView({ appointments }: { appointments: Appointment[] }) {
  ...
  const getAppsForDay = (day: Date) =>
    appointments.filter((a) => a.startAt && isSameDay(new Date(a.startAt), day));
            ...
            >
              ...
              <div className="mt-1 space-y-0.5">
                {apps.slice(0, 3).map((a) => (
                  ...
                  >
                    {a.startAt && format(new Date(a.startAt), "HH:mm")} {a.service.name}
                  </div>
                ))}
                {apps.length > 3 && (
                  <div className="text-[10px] text-gray-400">+{apps.length - 3} daha</div>
                )}
              </div>
            </div>
          );
