...
import { getTenantBranding, contrastText } from "@/modules/randevu/lib/branding";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: b.siteTitle ?? (b.name ? `${b.name} | Randevu Paneli` : "Randevu Paneli"),
    icons: {
...
export default async function RandevuDashboardLayout({ children, params }: Props) {
        ...
        <div className="h-16 flex items-center px-6 border-b border-slate-200 flex-shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-400 leading-none">Randevu Paneli</p>
            </div>
          </div>
        </div>
